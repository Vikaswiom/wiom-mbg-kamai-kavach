...
| **आज का इंस्टॉलेशन (TECHNICIAN screen — live TAS, not a snapshot)** | https://vikaswiom.github.io/wiom-mbg-kamai-kavach/daily-installation.html?technicianId=`<id>` |
...
| `daily-installation.html` | **Technician screen — the only page with NO data file.** Every other screen reads a baked JSON snapshot; this one calls TAS live per open: `GET csp-gateway-service.i2e1agents.in/public/install/candidates/by-executor?executorId=<id>`. Identity comes from the CleverTap profile property **`accountId`** (the creative reads it; the screen also accepts it as `?accountId=`). That one value is TAS's `executorId` — one id under two names — and there is **no `cspId` anywhere on this screen**. The CT token carries `default:"v"`, so a profile without the property substitutes the literal `v`; any single-character value is rejected as "no id" and the API is never called. Renders today's install tickets with a Call CTA per customer. `?demo=1` previews the UI with a sample list (no id, no API); `?live=1` forces the live call from disk. Events are `DailyInstall_*`, **one per action, never per card** — 12 tickets still raise one `Call_Clicked`, identified by `position`/`candidateId`, so the CT event catalogue never grows with the list. |
| `docs/inapp-daily-installation-wrapper.html` | The CleverTap creative for the screen above — paste-whole-file. Resolves `technicianId` from the profile (4 spellings, Liquid is case-sensitive), iframes the Pages URL, and relays the screen's events / dial / dismiss to the TOP frame. Needed because `inapp-settle-wrapper.html` keys on `cspid` and has no message listener. |
