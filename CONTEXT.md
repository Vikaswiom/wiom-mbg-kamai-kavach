...
| **आज का इंस्टॉलेशन** (technician's daily install list) | https://vikaswiom.github.io/wiom-mbg-kamai-kavach/daily-installation.html?technicianId=`<id>` |

**The CT campaign URL** (push / SMS / WhatsApp — CT substitutes the id into the link itself):

```
https://vikaswiom.github.io/wiom-mbg-kamai-kavach/daily-installation.html?technicianId={{Profile.technicianId}}
```

`technicianId` is the only variable and is TAS's `executorId` under another name — the page
forwards it as `?executorId=` to
`https://csp-tas-service.i2e1agents.in/api/internal/install/candidates/by-executor`, so the
list is fetched **live on every open**. Confirm the profile property's exact spelling in
CleverTap (Segments → any profile → Custom attributes) before sending: Liquid is
case-sensitive, and a property it cannot resolve is left as literal token text in the URL —
the page then reports `No_Technician_Id` with `idSource=none` rather than calling TAS.
⚠️ **Prerequisite:** TAS must send `Access-Control-Allow-Origin` for
`https://vikaswiom.github.io`, or the browser blocks the call and the screen shows its error
state (`Api_Failed`, `reason=network_or_cors`, with the exact `apiUrl`). If TAS cannot
allow-list it, set `CFG.PROXY_URL` to an Apps Script relay (same pattern as `docs/live-proxy/`)
...
| `daily-installation.html` | **आज का इंस्टॉलेशन — the technician-facing daily install list.** The FIRST screen keyed on `technicianId`, not `cspId`. Live data from TAS `GET /api/internal/install/candidates/by-executor?executorId=<id>` (CT's `technicianId` IS TAS's `executorId` — one id, two names). Renders one card per candidate: customer name, recomposed address (TAS ships `service_address` as a JSON-ENCODED string), landmark, slot (`00:00–23:59` = "पूरे दिन"), the phone, and a Call CTA. Phone is found by named key then by a bounded walk of the item, and the key it actually came from rides the `Viewed` event as `phoneKeys` — so the field name is confirmable from CT with no code change. **Every clickable surface raises exactly one CT event, none twice** — `Call_Clicked` (the whole customer: name, full + masked phone, address, landmark, area, city, pincode, slot, state, candidate + connection ids), `Card_Tapped` (body tap, masked phone only), `Dismissed`, `Lang_Switched`, `Retry_Clicked`; plus `Opened` / `Viewed` / `All_Called` / `Phone_Missing` / `Api_Failed` / `No_Technician_Id` / `Id_Echo_Mismatch`. Every event carries `technicianId` AND `executorId` (same value, both names). Full catalogue is the comment block at §4 of the page. ⚠️ **A Liquid token may appear ONLY in the four `<span>`s** — CleverTap parses the whole creative including HTML comments and script bodies, so a token in a comment is still parsed and can refuse to save; the bare pipe-sentinel style is a syntax error in this account (confirmed 11 Sep 2026). `cleanToken()` therefore builds its brace literals by concatenation instead of typing them. The CT profile property is tried under four spellings (`technicianId`, `technician_id`, `executorId`, `executor_id`) because Liquid is case-sensitive and a single guess is the likeliest reason a real device shows "profile incomplete" against a healthy API; the one that won rides every event as `idSource`. **A zero-install day closes the popup itself** (`CFG.CLOSE_IF_EMPTY`) after `Viewed` is away, so the in-app only actually appears when there is a customer to call — the campaign still records the reach via `Viewed` + `Auto_Closed_Empty`. `Api_Failed` carries the exact `apiUrl` called. **Dismissal fires every transport and returns early from none** — on Android `CTWebInterface` is injected into sub-frames, so inside the wrapper's iframe `dismissInAppNotification` exists and is callable but often silently does nothing; returning on its presence stranded the tap and made the ✕ look dead (the same trap as `triggerInAppAction`). The page relays to the wrapper first, also tries its local bridges, and the wrapper escalates to the `clevertap://` scheme if neither answers. Only one ✕ shows when embedded — the wrapper's, which asks the page to record `Dismissed` before closing, and closes anyway if the page never loads. Hi/En toggle. No id is baked into the page — `TECH_ID` starts empty and is only ever set from the CT profile or the URL, and the API is never called without one. Preview: `?technicianId=<id>` · `?live=1` (force the real API from disk) · `?nophone=1`. **Three deployment modes, no edit between them** — `inapp_wrapper` (framed by the creative), `inapp_pasted` (whole file in the CT editor) and **`link`** (a plain public URL opened from a CT campaign). The mode rides every event as `mode`. In `link` mode the two popup-dismissal behaviours are off: a zero-install day shows the empty state instead of auto-closing, the ✕ is hidden, and the page never navigates to `clevertap://closeInApp` — an unknown scheme in a browser, which would replace the technician's list with an error page. The `?technicianId=` param goes through `cleanToken()` as well, so a campaign URL whose token CT failed to substitute reports `No_Technician_Id` rather than sending the raw token on to TAS as the executorId. |
| `docs/inapp-daily-installation-wrapper.html` | **The CleverTap in-app creative for the above** — same thin-frame split as `inapp-settle-wrapper.html`: the creative holds the Liquid token and iframes the Pages page. Not cosmetic — in the in-app WebView the creative runs at `about:blank` (`Origin: null`, which TAS cannot allow-list), while the iframe runs at `https://vikaswiom.github.io`, a real origin it CAN. It also owns the CT bridge (`CTWebInterface` is reliably injected into the TOP frame only) and **queues** events the page raises before the bridge exists. |
